...
The base image generated by `scripts/build_refugios.sh` has a known issue:

*   **Desktop icons not marked as trusted:** Desktop `.desktop` launchers are not correctly marked as trusted by XFCE. When double-clicking, XFCE shows a warning dialog asking for confirmation before executing the application. The injected trust mechanism (SHA-256 checksum via GIO `metadata::xfce-exe-checksum`) is not recognized by XFCE. Workaround: right-click on the icon → "Allow launching". See details in the **[System Image Build Guide](System-Image-Build-EN#7-known-bug-desktop-icons-not-marked-as-trusted)**.
