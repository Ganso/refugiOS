...
### Añadido
- **Verificador de imágenes (`tests/verify_image.sh`):** Una sola orden que comprueba una imagen construida **antes de publicarla**: integridad del sistema de ficheros, presencia de los trece artefactos que debe contener y, con `--expand`, que al volcarla en un disco de 128 GB la partición raíz crece de verdad. Comprueba el artefacto, no la construcción, que es justo lo que faltaba: la 0.23 salió con todas las señales en verde (código 0, log registrando el enlace como creado, captura de un escritorio correcto) porque ninguna de ellas mira dentro de la imagen, y la expansión no se puede ejercitar en un disco del mismo tamaño que la imagen. Documentado en las guías de la suite de tests y en el procedimiento de publicación de `AGENTS.md`.
